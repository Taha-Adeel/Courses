...
\begin{align}
    \sigma &= \sum_{X \in \mathbb{R}} \left(  (E\left[ X-E[X] \right])^2 \pr{X} \label{varience} \right)\\ &= E[X^2] - E[X]^2
\end{align}
As variance is always positive (We can easily see that from (\ref{varience})), 
\begin{align}
    E[X^2] - E[X]^2 \geq 0
\end{align}
...
\end{enumerate}
\par \vspace{0.3cm}
$\therefore  E[X]$ and $E[X^2]$ cannot attain the values $2$ and $3$ (\textbf{Option B}) (As then the variance would be negative (Refer (\ref{negative_variance}))
\end{document}
