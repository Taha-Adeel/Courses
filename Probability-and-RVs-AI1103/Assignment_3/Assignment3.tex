...
\end{enumerate}
\section{Solution}
Consider the state space $S=\{1,2\}$, where $1$ represents her studying CS and $2$ represents her studying maths on a particular day.\\ 
...
\par The $\pr{X_{n+t}=j | X_{n}=i}$ is the $(i,j)$th position of $P^{\,t}$. Therefore $\pr{X_{2}=1 | X_{0}=1}$(As $n=2$ for Wednesday and $n=0$ for Monday) is the $(1,1)$th position of $P^2$.
\begin{align}
    P^2=\begin{bmatrix}
0.4 & 0.6 \\
0.4 & 0.6
...
%%The probability she studies CS on Wednesday($n=2$) can be found by($\because X_0=0$) :
%%\begin{align}
