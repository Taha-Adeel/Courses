...
set<tuple<string, string, string>> get_authors(string authors)
{
    vector<string> authorNames = split_authors(authors);
    set<tuple<string, string, string>> split_names;
    for(auto name : authorNames)
    {
        ...
        split_names.insert(make_tuple(first_name, middle_name, last_name));
    }
    return split_names;
}
...
void populateData(connection *C){
    ...
    set<tuple<string, string, string>> auth_names_list;
