...
std::ofstream log_file("RW-log.txt");						// The output file where the logs are stored
std::ofstream stats_file("RW-Average_time.txt");			// The output file where the waiting time stats are stored
...
sem_t rw_mutex;						// The semaphore to ensure writersand readers are mutually exculsive
